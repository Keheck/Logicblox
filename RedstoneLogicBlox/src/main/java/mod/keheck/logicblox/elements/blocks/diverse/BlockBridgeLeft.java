...
public class BlockBridgeLeft extends GateBase implements NoActiveState
{
    public BlockBridgeLeft(String name)
    {
        super(name);
    }
    ...
    /**
     * returns true if the given side is an input (either the opposite side or the left)
     */
    private boolean isInputSide(EnumFacing side, IBlockState state)
    {
        return state.getValue(FACING).getOpposite() == side || state.getValue(FACING).rotateYCCW() == side;
    }
    ...
    {
        return ItemInit.ITEM_DIVERSE_BRIDGE_LEFT;
    }

    @Override
    protected IBlockState getPoweredState(IBlockState unpoweredState)
    {
        return BlockInit.DIVERSE_BRIDGE_LEFT.getDefaultState().withProperty(FACING, unpoweredState.getValue(FACING));
    }

    @Override
    protected IBlockState getUnpoweredState(IBlockState poweredState)
    {
        return BlockInit.DIVERSE_BRIDGE_LEFT.getDefaultState().withProperty(FACING, poweredState.getValue(FACING));
    }
